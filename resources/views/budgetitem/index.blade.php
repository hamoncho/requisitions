                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th
                                        ...
                                        Name</th>
                                    <th
                                        class="hidden sm:block px-2 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                        Actions</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($budgetItems as $item)
                                    <tr>
                                        <td class="px-2  whitespace-nowrap font-semibold text-gray-700">
                                            {{ $item->full_code }}</td>
                                        ...
                                        <td class="hidden sm:block px-2 whitespace-nowrap space-x-2">
                                            <a href="{{ route('budget_item.edit', $item) }}"
                                                class="inline-flex items-center px-4 py-2 bg-blue-950 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-900 focus:bg-blue-900 active:bg-blue-950 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                                Edit
                                            </a>

                                            <form method="POST" action="{{ route('budget_item.destroy', $item) }}"
                                                class="inline-block"
                                                onsubmit="return confirm('Are you sure you want to delete this item?');">
                                                @csrf
                                                @method('DELETE')
